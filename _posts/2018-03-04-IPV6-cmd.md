...
comments: true
share: true
title: IPv6 系列 -- 利用 IPv6 公共 DNS 實現免 VPN 機智上網 \ 步驟詳解 一
...
4. 上半部分的 IP 板塊根據運營商要求自行調整，校園網是自動獲取，主要更改下半部分的 DNS 選項，勾選 `Use the following DNS server address:`，在上下兩欄中分別鍵入：`240c::6666` 和 `2001:4860:4860::8888` ，按確認返回
...
2. 取消勾選 `DNS 自動獲取`，勾選 `手動設置`，輸入 `240c::6666,2001:4860:4860::8888` 
3. 享受高速的國際互聯網沖浪吧。
