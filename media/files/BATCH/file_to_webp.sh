...
# If image size > 50KB, convert to webp.
SIZE=5120

# Set convert quality 80.
QUALITY=50

# Convert image to webp:
FILE_LIST=`find $DIR* | egrep "^*\.(png|jpg|JPG|jpeg)$"`
...
find "$DIR" -name '*.gif' -exec sh -c 'ffmpeg -i "$0" "${0%%.gif}.webm"' {} \;

# Convert videos to webm:
# find "$DIR" -name '*.mp4' -exec sh -c 'ffmpeg -i "$0" -c:v libvpx -crf 10 -b:v 1M -c:a libvorbis "${0%%.mp4}.webm"' {} \;
...
classifier -st jpg -sf "JPG" -d "$DIR" -o "$DIR"
classifier -st png -sf "PNG" -d "$DIR" -o "$DIR"
classifier -st gif -sf "GIF" -d "$DIR" -o "$DIR"
classifier -st mp4 -sf "MP4" -d "$DIR" -o "$DIR"
classifier -st webp -sf "WEBP" -d "$DIR" -o "$DIR"
classifier -st webm -sf "WEBM" -d "$DIR" -o "$DIR"